...
int main(int argc, char *argv[]) {
    ...
    int running = 0;     // número de tarefas em execução
    int next_task = 0;   // próxima tarefa a lançar

    while (next_task < num_tasks || running > 0) {

        /* Lançar novas tarefas até atingir o limite */
        ...
        /* Espera pela conclusão de uma tarefa */
        wait(NULL);
        running--;
    }
